...
function add_distortion_mod() {
    ...
    // This should be linked to an oscillator, not making one itself
    const oscSaw = audioContext.createOscillator();
    oscSaw.type = 'sinewave';
    oscSaw.frequency.value = 220;
...
function distortion_curve(amount) {
    ...
    for ( ; i < n_samples; ++i) {
        ...
        x = i * 10 / n_samples - 1;
        curve[i] = (30 + k) * x * 20 * degree / (Math.PI + k * Math.abs(x));
    }
